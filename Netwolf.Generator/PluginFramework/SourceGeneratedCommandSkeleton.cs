...
[assembly: SourceGeneratedCommand(typeof({command.ContainerType}), typeof({generatedTypeName}), typeof({resultType}))]
...
");
        ...
        if (command.IsMethodAsync)
        {
            sb.AppendLine("return new(result);");
        }
        else
        {
            sb.AppendLine("return Task.FromResult(new(result));");
        }
