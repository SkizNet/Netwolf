...
namespace Netwolf.Generator.Unicode;

[Generator]
public class UnicodeDataGenerator : IIncrementalGenerator
{
...
");
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        var eastAsianWidth = context.AdditionalTextsProvider
            .Where(static text => text.Path.EndsWith("EastAsianWidth.txt"))
            .Combine(assembly)
            .Where(static model => model.Right == "Netwolf.Unicode")
            .SelectMany(static (model, token) => model.Left.GetText(token)?.ToString().Split(['\r', '\n'], StringSplitOptions.RemoveEmptyEntries) ?? [])
            .Where(static line => !line.StartsWith("#"))
        ...
        var lineBreak = context.AdditionalTextsProvider
            .Where(static text => text.Path.EndsWith("LineBreak.txt"))
            .Combine(assembly)
            .Where(static model => model.Right == "Netwolf.Unicode")
            .SelectMany(static (model, token) => model.Left.GetText(token)?.ToString().Split(['\r', '\n'], StringSplitOptions.RemoveEmptyEntries) ?? [])
            .Where(static line => !line.StartsWith("#"))
        ...
        context.RegisterSourceOutput(eastAsianWidth, static (sourceProductionContext, model) =>
        {
            if (!model.IsEmpty)
            {
                sourceProductionContext.AddSource("UnicodeProperty.EastAsianWidth.g.cs",
                    GetGenericLookupSourceText(model, "EastAsianWidth", "Neutral").ToString());
            }
        });

        context.RegisterSourceOutput(lineBreak, static (sourceProductionContext, model) =>
        {
            if (!model.IsEmpty)
            {
                sourceProductionContext.AddSource("UnicodeProperty.LineBreakClass.g.cs",
                    GetGenericLookupSourceText(model, "LineBreakClass", "Unknown").ToString());
            }
        });
    }
