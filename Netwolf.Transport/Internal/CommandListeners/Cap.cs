...
[CommandListener]
internal sealed class Cap : IAsyncCommandListener, INetworkInitialization, IDisposable
{
    public IReadOnlyCollection<string> CommandFilter => ["CAP"];
    ...
    public Cap(ILogger<INetwork> logger, CommandListenerRegistry registry)
    {
        Logger = logger;
        Registry = registry;
    ...
    {
        network.NetworkConnecting += (sender, args) => PendingCaps[args.Network] = [];
        network.NetworkDisconnected += (sender, args) => PendingCaps.Remove(args.Network);
    }
    ...
    public async Task ExecuteAsync(CommandEventArgs args)
    {
        ...
        switch (command.Args[1])
        {
            case "LS":
            case "NEW":
                await HandleNewCap(network, command, args.Token);
                break;
            case "ACK":
            case "LIST":
    ...
    private async Task HandleNewCap(INetwork network, ICommand command, CancellationToken cancellationToken)
    {
        string caps;
        bool final = false;
        ...
        if (command.Args[1] == "LS" && network.IsConnected)
        {
            return;
        }
